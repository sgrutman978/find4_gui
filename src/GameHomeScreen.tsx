...
import GameSuiteClient, { baseUrl, fetchEvents, /*GetProfile,*/ port } from './sui_controller';
...
function GameHomeScreen() {
	...
	const getMyGamesObjectsHelper = async (game: any) => {
		let opponentAddy = (game.p1 == gsl.myAddy ? game.p2 : game.p1);
			return await getProfileFromServer(opponentAddy).then(async (profile) => {
				...
				return (<div className="existingGame" onClick={() => {window.location.href = `/app/game/${game.id}`}}>
					{(!game.winner ? (((game.type == 1 && game.currentPlayerTurn == 1) || 
						(game.type == 2 && ((game.currentPlayerTurn == 1 && gsl.myAddy == game.p1) || (game.currentPlayerTurn == 2 && gsl.myAddy == game.p2)))) ? 
						...
						(game.type == 2 && ((game.winner == 1 && gsl.myAddy == game.p1) || (game.winner == 2 && gsl.myAddy == game.p2)))) ? 
	...
	const getMyGamesObjects = async (): Promise<any> => {
		...
		const results = await Promise.all(myGames.map(async (game) => {
			...
			if(  !(!game.winner && ((game.type == 1 && game.currentPlayerTurn == 1) || (game.type == 2 && ((game.currentPlayerTurn == 1 && gsl.myAddy == game.p1) || (game.currentPlayerTurn == 2 && gsl.myAddy == game.p2)))))){
	...
	function customSort() {
		return myGames.sort((a, b) => {
			...
			if (!aHasWinner && !bHasWinner) {
				const aIsMyTurn = ((a.type == 1 && a.currentPlayerTurn == 1) || (a.type == 2 && ((a.currentPlayerTurn == 1 && gsl.myAddy == a.p1) || (a.currentPlayerTurn == 2 && gsl.myAddy == a.p2))));
				const bIsMyTurn = ((b.type == 1 && b.currentPlayerTurn == 1) || (b.type == 2 && ((b.currentPlayerTurn == 1 && gsl.myAddy == b.p1) || (b.currentPlayerTurn == 2 && gsl.myAddy == b.p2))));
  ...
  return (
		<div className="gameHomeScreen">
			...
			<div className="logo_app_home">
				<div className="gameHomeLogoDiv" onClick={() => window.location.href = '/info'}>
